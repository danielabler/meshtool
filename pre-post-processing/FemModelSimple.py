...
class FemModel():
    ...
    def export_to_abaqus(self, path_to_file=None, ):
        ...
        for group_name in ["ElementBlockIds"]:
            groups = chf.get_unique_integer_list_from_vtu_array(self.mesh, "cell_array", group_name)
            print("Creating Elementsets for subdomains: %s"%groups)
            group_elset_dict = {}
            for group_id in groups:
                group_id_full = "%s_%03i"%(group_name, group_id)
                element_set_name = "ELEMENTS_%s_%03i"%(group_name, group_id)
                groups_name   = "GROUP_%s_%03i"%(group_name, group_id)
                selected_cell_vtk_ids = np.where(self.mesh_wrapped.CellData[group_name]==group_id)[0]
                self.__write_abq_set(sets_file, element_set_name, selected_cell_vtk_ids, set_type="element")    
                group_temp_dict = {}
                group_temp_dict["element_set_name"] = element_set_name
